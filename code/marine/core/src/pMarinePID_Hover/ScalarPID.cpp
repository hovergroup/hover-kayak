...
//-------------------------------------------------------------------
bool ScalarPID::Run(double measurement, double target, double dfErrorTime, double &dfOut, bool isAngle) {
	if ( isAngle )
		m_dfe = angle180(measurement-target);
	else
		m_dfe = target-measurement;
	...
	if (m_nIterations++ != 0) {
		...
		if (m_dfDT < 0) {
		    ...
		    m_oldMeasurement = measurement;
		    Log();
		    return true;
		}

		double measurement_diff;
		if ( isAngle )
			measurement_diff = angle180(measurement-m_oldMeasurement);
		else
			measurement_diff = measurement-m_oldMeasurement;

		m_dfeDiff = (measurement_diff+m_tau*m_dfeDiff)/(m_tau+m_dfDT);
	} else {
	...
	//do pid control
	m_dfOut = (m_dfKp * m_dfe) + (m_dfKd * m_dfeDiff) + (m_dfKi * m_dfeSum);
...
bool ScalarPID::Log() {
	...
	if (m_bLog) {
		if (!m_LogFile.is_open()) {
			string sName = MOOSFormat("%s%s%s.pid", m_sLogPath.c_str(),
					m_sName.c_str(), MOOSGetTimeStampString().c_str());
