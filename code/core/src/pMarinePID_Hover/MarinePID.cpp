...
bool MarinePID::OnStartUp() {
    ...
    for (p = sParams.begin(); p != sParams.end(); p++) {
        ...
        if (param == "SPEED_FACTOR")
            ...
            if (sLine == "quiet")
                m_verbose = "quiet";
        } else if (param == "SPEED_CONTROLLER")
            speed_controller = sLine;
    }

    if (speed_controller == "factor") {
        cout << "Using speed factor control" << endl;
        m_speed_control_type = FACTOR;
    } else if (speed_controller == "pid") {
        cout << "Using pid speed controller" << endl;
        m_speed_control_type = PID;
    } else if (speed_controller == "fit_pid") {
        cout << "Using fit+pid speed controller" << endl;
        m_speed_control_type = FIT_PID;
    } else {
        cout << "Speed control type malconfig" << endl;
        return false;
    }
...
bool MarinePID::handleYawSettings() {
    ...
    if ((!m_MissionReader.GetConfigurationParam("YAW_PID_INTEGRAL_LIMIT",
            yaw_pid_ilim))
            && (!m_MissionReader.GetConfigurationParam("YAW_PID_KI_LIMIT",
                    yaw_pid_ilim))) {
...
bool MarinePID::handleSpeedSettings() {
    ...
    m_pengine.setSpeedControlType(m_speed_control_type);
    if (m_speed_control_type == FACTOR) {
        // set speed factor and a placeholder PID

        m_pengine.setSpeedFactor(m_speed_factor);

        ScalarPID spdPID;
        spdPID.SetGains(0, 0, 0);
        spdPID.SetLimits(0, 100);
        m_pengine.setPID(1, spdPID);

        return true;
    } else if (m_speed_control_type == FIT_PID) {
        // get linear fit info

        double spd_slope, spd_offset, angle_limit, time_delay;
        if (!m_MissionReader.GetConfigurationParam("SPEED_SLOPE", spd_slope)) {
            MOOSDebugWrite("SPEED_SLOPE not found in Mission File");
            ok = false;
        }
        if (!m_MissionReader.GetConfigurationParam("SPEED_OFFSET", spd_offset)) {
            MOOSDebugWrite("SPEED_OFFSET not found in Mission File");
            ok = false;
        }
        if (!m_MissionReader.GetConfigurationParam("ANGLE_LIMIT", angle_limit)) {
    ...
    if (!m_MissionReader.GetConfigurationParam("SPEED_PID_INTEGRAL_LIMIT",
            spd_pid_ilim)) {
...
bool MarinePID::handleDepthSettings() {
    ...
    m_depth_control = ((depth_control_str == "true")
            || (depth_control_str == "1") || (depth_control_str == "yes"));
#endif
    ...
    if (!m_MissionReader.GetConfigurationParam("Z_TO_PITCH_PID_KP",
            z_top_pid_Kp)) {
        MOOSDebugWrite("Z_TO_PITCH_PID_KP not found in Mission File");
        ok = false;
    }
    if (!m_MissionReader.GetConfigurationParam("Z_TO_PITCH_PID_KD",
            z_top_pid_Kd)) {
        MOOSDebugWrite("Z_TO_PITCH_PID_KD not found in Mission File");
        ok = false;
    }
    if (!m_MissionReader.GetConfigurationParam("Z_TO_PITCH_PID_KI",
            z_top_pid_Ki)) {
        MOOSDebugWrite("Z_TO_PITCH_PID_KI not found in Mission File");
        ok = false;
    }
    if (!m_MissionReader.GetConfigurationParam("Z_TO_PITCH_PID_INTEGRAL_LIMIT",
            z_top_pid_ilim)) {
        MOOSDebugWrite(
                "Z_TO_PITCH_PID_INTEGRAL_LIMIT not found in Mission File");
        ok = false;
    }
    ...
    if (!m_MissionReader.GetConfigurationParam("PITCH_PID_INTEGRAL_LIMIT",
            pitch_pid_ilim)) {
